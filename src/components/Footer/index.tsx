'use client';

import React, { useState } from 'react';
import { Home, ChevronDown, Info } from 'lucide-react';
import './Footer.css'; // Import the CSS file

const Footer = () => {
  const [isOpen, setIsOpen] = useState(false);
  
  const toggleFooter = () => {
    setIsOpen(!isOpen);
  };
  
  return (
    <div className="footer-container">
      {/* Main footer with sliding animation */}
      <div className={`sliding-footer ${isOpen ? 'open' : ''}`}>
        <div className="footer-content">
          <ul className="social-icons">
            <li className="github-icon">
              <a href="#" title="GitHub" target="_blank" rel="noopener noreferrer">
                <div className="icon-circle">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path>
                  </svg>
                </div>
              </a>
            </li>
            <li className="linkedin-icon">
              <a href="#" title="LinkedIn" target="_blank" rel="noopener noreferrer">
                <div className="icon-circle">
                  <span>in</span>
                </div>
              </a>
            </li>
            <li className="email-icon">
              <a href="#" title="Email">
                <div className="icon-circle">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                    <polyline points="22,6 12,13 2,6"></polyline>
                  </svg>
                </div>
              </a>
            </li>
          </ul>
        </div>
      </div>
      
      {/* Footer dock with left and right corner icons */}
      <div className="footer-dock">
        <div className="corner-icons">
          <div className="left-corner-icon">
            <a href="/" title="Home">
              <div className="icon-circle">
                <Home size={16} />
              </div>
            </a>
          </div>
          <div className={`right-corner-icon ${isOpen ? 'open' : ''}`} onClick={toggleFooter}>
            <div className="icon-container">
              <div className="icon-circle">
                {isOpen ? (
                  <ChevronDown
                    className="icon-down"
                    size={16}
                  />
                ) : (
                  <Info
                    className="icon-info"
                    size={16}
                  />
                )}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Footer;